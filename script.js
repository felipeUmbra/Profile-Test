...
import { fetchQuestions, generateBig5ResultHTML, generateDISCResultHTML } from '/JS/questionsHandler.js';
import { saveProgressToLocalStorage, saveProgressToDatabase, saveResultToDatabase } from '/JS/dataHandler.js';
...
function saveProgress() {
    ...
    saveProgressToDatabase(currentLang).catch(() => {
...
export let currentTestQuestions = [];

// Global State
export let currentQuestionIndex = 0;
export let scores = { D: 0, I: 0, S: 0, C: 0 };
export let mbtiScores = { E: 0, I: 0, S: 0, N: 0, T: 0, F: 0, J: 0, P: 0 };
export let big5Scores = { O: 0, C: 0, E: 0, A: 0, N: 0 };
...
function updateIndexStaticText() {
    try {
        // Update main content
        document.getElementById('main-title').textContent = translateUtilIndex('mainTitle');
        document.getElementById('subtitle').textContent = translateUtilIndex('subtitle');
        
        // Update test links
        document.getElementById('disc-test').textContent = translateUtilIndex('discTest');
        document.getElementById('disc-subtitle').textContent = translateUtilIndex('discSubtitle');
        document.getElementById('mbti-test').textContent = translateUtilIndex('mbtiTest');
        document.getElementById('mbti-subtitle').textContent = translateUtilIndex('mbtiSubtitle');
        document.getElementById('big5-test').textContent = translateUtilIndex('big5Test');
        document.getElementById('big5-subtitle').textContent = translateUtilIndex('big5Subtitle');
        
        // Update results section
        document.getElementById('results-title').textContent = translateUtilIndex('resultsTitle');
        document.getElementById('clear-results-btn').textContent = translateUtilIndex('clearResults');
        
        // Update footer
        document.getElementById('footer-text-1').textContent = translateUtilIndex('footerText1');
        document.getElementById('footer-text-2').textContent = translateUtilIndex('footerText2');
    } catch (error) {
        ...
        dimensions.forEach(factor => {
            ...
            if (percentage >= 70) {
                interpretation = factor === 'N' ? 
                    { en: "High - May experience frequent emotional distress", pt: "Alto - Pode experimentar angústia emocional frequente", es: "Alto - Puede experimentar angustia emocional frecuente" } :
                    { en: "High - Strong tendency in this trait", pt: "Alto - Forte tendência neste traço", es: "Alto - Fuerte tendencia en este rasgo" };
            } else if (percentage >= 30) {
                interpretation = { en: "Moderate - Balanced level of this trait", pt: "Moderado - Nível equilibrado deste traço", es: "Moderado - Nivel equilibrado de este rasgo" };
            } else {
                interpretation = factor === 'N' ?
                    { en: "Low - Emotionally stable and resilient", pt: "Baixo - Estável emocionalmente e resiliente", es: "Bajo - Estable emocionalmente y resiliente" } :
                    { en: "Low - Limited tendency in this trait", pt: "Baixo - Tendência limitada neste traço", es: "Bajo - Tendencia limitada en este rasgo" };
            }
...
function createBig5FriendlyDescription(traitAnalysis, date) {
    const t = indexInterfaceTranslations[currentLang];
...
function displayFullResult(testType, resultData) {
    ...
    switch (testType) {
        ...
        case 'MBTI':
            ...
            break;
        case 'BIG5':
            resultHTML = generateBig5ResultHTML(resultData, currentLang);
            break;
    }
...
function initIndexPage() {
    try {
        ...
        if (clearBtn) {
            ...
            clearBtn.setAttribute('aria-label', translateUtilIndex('clearResults'));
        }
